...
# Kairo: A Readable, High-Performance, Low-Level Language.
...
- [Kairo: A Readable, High-Performance, Low-Level Language.](#kairo-a-readable-high-performance-low-level-language)
  ...
  - [Table of Contents](#table-of-contents)
    ...
    - [Features of Rust, Zig, Odin, Nim, and Kairo](#features-of-rust-zig-odin-nim-and-kairo)
    ...
    - [Kairo ABI and FFI System](#kairo-abi-and-ffi-system)
    - [Kairo FFI System](#kairo-ffi-system)
      - [Core Features](#core-features)
      - [The Kairo ABI Extension file defines how Kairo interacts with foreign languages, including:](#the-kairo-abi-extension-file-defines-how-kairo-interacts-with-foreign-languages-including)
    ...
    - [What constitutes as Pure Kairo Code?](#what-constitutes-as-pure-kairo-code)
    - [Kairo's Cache System](#kairos-cache-system)
  - [Kairo Roadmap](#kairo-roadmap)
...
Kairo uses a hybrid error-handling model with **questionable types** (`T?`), which can hold a value (`T`), `null`, or a `panic`. This supports both compile-time and runtime error handling.
...
Kairo's syntax and semantics are inspired by C++ to ease adoption, with distinct features for clarity and safety. It also integrates directly with C++ codebases.

### Language Features

- **Templates**: Both Kairo and C++ use template mechanisms to enable generic programming. Kairo has the same core principles and introduces a different syntax `fn <T> name(param: T)` that is more intuitive, while still allowing for powerful type abstractions, along with a greater support for type bounds (i.e. `fn <T impl Comparable, N> name(param: [T; N]) if N > 21`).
...
<div>
  ...
  <img src="../assets/cxx-like-kairo-ex.png" width="47%" alt="Kairo Example" style="float:right; margin-right: 3%; vertical-align: top;">
  <img src="../assets/bumper.png" width="100%" alt="Bumper">
</div>

### C++ Interoperability

Kairo integrates with C++ via its Foreign Function Interface (FFI), enabling direct use of C++ libraries and types.

- **Header Inclusion**: Import C++ headers or C++20 modules.
- **Type Mapping**: C++ types (e.g., `std::vector<int>`) are mapped to Kairo types, with compiler-generated bindings for memory and function calls.
- **Function Calls**: Call C++ functions directly, with FFI handling calling conventions and type conversions.
- **Declaration Style**: Use C++ classes in Kairo without redefinition, as shown below:
- **Bidirectional Calls**: Kairo functions can be called from C++ with full interoperability, using the Itanium C++ ABI and the C ABI for seamless integration.

<div>
  <img src="../assets/interoperability-with-cxx-kairo.png" width="54%" alt="C++ Header" style="float:left; margin-left: 3%; vertical-align: top;">
  <img src="../assets/interoperability-with-cxx-hh.png" width="44%" alt="Kairo Implementing C++" style="float:right; margin-right: 3%; vertical-align: top;">
  <img src="../assets/bumper.png" width="100%" alt="Bumper">
</div>
...
### Features of Rust, Zig, Odin, Nim, and Kairo

This section outlines the technical features of Rust, Zig, Odin, Nim, and Kairo in key areas: object-oriented programming, memory safety, language features, and syntax. Each language's approach is described factually to highlight their design choices.
...
- **Kairo**: Same as C++ with classes, inheritance, and polymorphism. Supports operator overloading and method definitions within classes. Classes can implement interfaces for polymorphic behavior, also supports zero-cost interfaces.
...
- **Kairo**: Uses an Advanced Memory Tracking (AMT) system that automatically manages memory safety and with performance at compile time. It supports both automatic and manual memory management, with smart pointers for safety and raw pointers for performance.
...
- **Kairo**: Provides a macro system with `macro name!(params) { ... };` for compile-time code generation. We also support procedural macros for advanced meta-programming with: `macro @name(params) { ... };`.
...
### Kairo ABI and FFI System

Kairo's Foreign Function Interface (FFI) system allows seamless integration with other languages like Python, Java, or Rust, making it ideal for projects that combine multiple languages. The FFI is flexible, extensible, and designed to maintain Kairo's performance and safety guarantees when interacting with foreign code.

### Kairo FFI System

The FFI system allows Kairo to call functions and use data types defined in other languages while ensuring compatibility and efficiency.

#### Core Features

- **Standard ABI Support**: Kairo supports the C ABI and Itanium C++ ABI by default, allowing direct calls to C and C++ libraries.
- **Kairo ABI**: A custom, extensible ABI designed for performance and safety. It handles function calls, data types, memory management, and calling conventions for foreign languages.
- **Automatic Bindings**: The Kairo compiler generates bindings and wrappers based on a **Kairo ABI Extension** file, reducing manual work for developers.
- **Pure Kairo Code**: All FFI interactions are lowered to an internal representation called Pure Kairo Code, which is C ABI-compatible, ensuring portability and optimization.

#### The Kairo ABI Extension file defines how Kairo interacts with foreign languages, including:

- **Function Signatures**: Maps foreign function prototypes to Kairo types, including arguments and return values.
- **Data Types**: Defines how foreign primitive types (e.g., Python's `PyObject*` or Rust's `Box<T>`) are represented in Kairo.
...
- **Flexibility**: The Kairo ABI can be extended to support languages like Python, Java (via JNI), and Rust, with community-driven extensions.
...
AMT is Kairo's memory management system, handling ownership and borrowing without requiring a garbage collector (GC). It uses a dedicated intermediate representation (IR) to track memory states at compile time, ensuring safety while preserving performance in mutable contexts.
...
- **No Garbage Collector**: AMT eliminates the need for a GC by resolving ownership at compile time or via smart pointers. This avoids GC pauses, making Kairo useable in real-time systems, performance-critical applications, and mutable-by-default workflows where frequent updates to data are common.
...
- **Module Structure**: A Vial file contains:
  ...
  - Preprocessed Kairo code (Pure Kairo Code), with platform-agnostic steps (e.g., macro expansion, type checking) completed.
  - Cached compilation outputs for reuse.

- **Compilation Process**:

  - The Kairo compiler preprocesses code into a Vial, performing tasks like macro expansion and borrow checking.
  - During final compilation, only platform-specific steps (e.g., generics instantiation, `eval` execution, which are stored septate form the pure kairo to allow recompile each time.) Emitting LLVM IR with minimal checks, since all checks are done at vial creation time before and all omissions is cached in the vial.
...
- **Cross-Language Support**: Vial supports other languages via language-specific modules. For example:
  ...
  - To compile for Python: `kairo --le=python ...` or `kairo --emit-le=python /path/to/vial -o/output/folder`.
...
- **Interoperability**: Vial allows Kairo code to integrate with languages like Python or C++ by emitting runtime-compatible binaries and wrappers, leveraging Kairo's FFI for function and type bindings.
...
### What constitutes as Pure Kairo Code?

Pure Kairo Code is the intermediate representation (IR) generated by the Kairo compiler after preprocessing and before final compilation. It does not include platform-specific details, runtime dependencies, generics, macros, or any other features that require platform-specific handling or resolution. Pure Kairo Code is designed to be platform-agnostic, allowing it to be compiled once and reused and linked to.

### Kairo's Cache System

Kairo's cache system is designed to optimize the compilation process by storing compiled Pure Kairo Code artifacts, seprate from Impure Kairo Code; Subsequently, the compiler can reuse the artifacts for faster builds while need a compilation unit only for Impure Kairo Code, this allows for significantly faster compilation times, especially for large projects with many dependencies.
...
------- | -------------------- | -------------------------------------------------------- | ----------------------------------------------------------------------------------------- | ---------- | ------
Phase 1 | Foundation           | C++ Bootstrap                                            | Finalize the C++ bootstrap to handle a minimal Kairo subset for writing a compiler.       | 1 month    | 🟩
Phase 2 | Frontend             | ...                                                      | Build a full Kairo frontend in Kairo to parse all syntax and generate Pure Kairo Code IR. | 3–4 months | 🟧
...     | ...                  | Lexer/Parser                                             | ...                                                                                       | ...        | 🟧
...     | ...                  | Error handling                                           | ...                                                                                       | ...        | 🟧
...     | ...                  | Type checking                                            | ...                                                                                       | ...        | 🟥
...     | ...                  | Type bounds checking                                     | ...                                                                                       | ...        | 🟥
...     | ...                  | Emit Pure Kairo Code IR                                  | ...                                                                                       | ...        | 🟥
...     | ...                  | Test syntax, errors, basic C++ FFI interop               | ...                                                                                       | ...        | 🟥
Phase 3 | AMT & Borrow Checker | ...                                                      | Add Advanced Memory Tracking and borrow checker in Kairo for memory safety.               | 3–4 months | 🟥
...
Phase 4 | Backend              | ...                                                      | Build a Kairo backend to turn Pure Kairo Code IR into LLVM IR and binaries.               | 4–5 months | 🟥
...     | ...                  | Map IR to LLVM IR                                        | ...                                                                                       | ...        | 🟥
...
...     | ...                  | Vial for Pure Kairo Code, AMT state, dependency caching  | ...                                                                                       | ...        | 🟥
...     | ...                  | ...                                                      | Cross-language output (kairo --le=python)                                                 | ...        | 🟥
...     | ...                  | Test C++ calls, Python binaries, safety checks           | ...                                                                                       | ...        | 🟥
...     | ...                  | Optimize Vial for 50% faster incremental builds          | ...                                                                                       | ...        | 🟥
Phase 6 | Self-Hosting         | ...                                                      | Make the Kairo compiler compile itself, no C++ bootstrap needed.                          | 2–3 months | 🟥
...     | ...                  | Port remaining bootstrap code to Kairo                   | ...                                                                                       | ...        | 🟥
...     | ...                  | Compile Kairo compiler with itself                       | ...                                                                                       | ...        | 🟥
...
xmake
```
7. Add the Kairo binary to your PATH (optional) it would be located in `./build/release/.../bin`

Congratulations! You have successfully installed Kairo. You can now run the Kairo compiler by executing `kairo` in your terminal or command prompt.
