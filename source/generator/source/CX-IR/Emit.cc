...
__CXIR_CODEGEN_BEGIN {
    ...
    void CXIRBuilder::add_to_source_map(const CX_Token *token) {
        if (this->source_map == nullptr) {
            return;
        }

        if (token == nullptr) {
            this->source_map->add_loc(SourceLocation{.kairo = {this->kairo_line, this->kairo_col},
    ...
    CXIRBuilder::CXIRBuilder(const size_t len, SourceMap *source_map) {
    ...
    CXIRBuilder &CXIRBuilder::add_line() {
        this->cxir += "\n";
        ++this->cxir_line;
        this->cxir_col = 1;
        return *this;
    ...
    CXIRBuilder &CXIRBuilder::add_line(const string &str) {
        this->cxir += str + "\n";
    ...
    CXIRBuilder &CXIRBuilder::add_line(const CX_Token &token) {
    ...
    CXIRBuilder &CXIRBuilder::add_line(const std::unique_ptr<CX_Token> &token) {
        if (token == nullptr) {
            return *this;
        }
    ...
    CXIRBuilder &CXIRBuilder::add(const string &str) {
    ...
    CXIRBuilder &CXIRBuilder::add(const CX_Token &token) {
        bool new_line_added = token.get_value()[0] == '#';
    ...
    CXIRBuilder &CXIRBuilder::add(const std::unique_ptr<CX_Token> &token) {
        if (token == nullptr) {
            return *this;
        }
    ...
    CXIRBuilder &CXIRBuilder::add_line_marker(
        const size_t line_num, const string &file_macro, const string &file_name) {
        this->add_line();
        ...
        if (!file_name.empty() && !file_macro.empty()) {
            ...
            // anchor: pin this file to its starting cxir position
            source_map->add_loc(
                SourceLocation{.kairo = {line_num, 1}, .cxir = {this->cxir_line, 1}});
    ...
    CXIRBuilder &CXIRBuilder::add_macro(const string &str) {
        this->add_line();
        this->add_line(str);
        return *this;
    };

    CXIRBuilder &CXIRBuilder::operator+=(const string &str) {
        this->add(str);
        return *this;
    };
    CXIRBuilder &CXIRBuilder::operator+=(const CX_Token &token) {
        this->add(token);
        return *this;
    };
    CXIRBuilder &CXIRBuilder::operator+=(const std::unique_ptr<CX_Token> &token) {
        this->add(token);
        return *this;
    };
    ...
    [[nodiscard]] string CXIRBuilder::build() const {
        this->source_map->finalize();
        return this->cxir;
    };
    [[nodiscard]] string CXIRBuilder::get_file_name() const { return this->file_name; };

    void reset_cxir_statics() {
        f_index                                 = 0;
        generator::CXIR::CXIRBuilder::cxir_line = 1;
        generator::CXIR::CXIRBuilder::cxir_col  = 1;
    }
    ...
    std::string CXIR::generate_CXIR() const {
        ...
        cxir.add_line_marker(1,
                             file_macros.begin()->second,
        ...
        for (size_t i = 0; i < tokens.size(); ++i) {
            ...
            if ((token->get_value() == "#if" || token->get_type() == cxir_tokens::CXX_PP_IF) ||
                ...
                // in this case we get the line from the next to next token since '#if (' - dont
                // have a line number
                ++line_num;
                ++i;  // skip #if or #elif
                ...
                cxir.add_line().add(token);  // add the #if or #elif
