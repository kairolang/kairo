...
int main(int argc, char **argv) {
    // we do sum really basic js a for loop looking for --lsp-mode
    for (int i = 1; i < argc; ++i) {
        if (std::string(argv[i]) == "--lsp-mode") {
            return kairo::lsp::start_lsp_server(argc, argv);
        }
    }
